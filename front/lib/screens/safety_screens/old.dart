...
class ElderlyContraScreen extends StatefulWidget {
  const ElderlyContraScreen({Key? key}) : super(key: key);

  @override
  State<ElderlyContraScreen> createState() => _ElderlyContraScreenState();
}

class _ElderlyContraScreenState extends State<ElderlyContraScreen> {
  ...
  Future<void> _startIntro() async {
    ...
    await _tts.speak("노인 금기 약물 확인 화면입니다. 약 이름을 말씀해주세요.");
  ...
  /// ✅ 선택한 약의 노인금기 확인
  Future<void> _checkElderlyContra(String input) async {
    ...
    await Vibration.vibrate(duration: 120);
    await _tts.speak("${matched['itemName']}을 선택하셨습니다. 노인 금기 정보를 불러옵니다.");
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _analyzeElderlyData(results);
      } else {
  ...
  /// 📊 노인금기 검사 로직
  Future<void> _analyzeElderlyData(Map<String, dynamic> results) async {
    bool found = false;
    for (var entry in results.values) {
      final detail = entry['dur']?['elderlyTaboo'] ?? [];
      if (detail.isNotEmpty) {
        found = true;
        for (var d in detail) {
          final name = d['ITEM_NAME'] ?? '약물명 미상';
          final content = d['PROHBT_CONTENT'] ?? '내용 없음';
          await _tts.speak("$name은 노인 금기 약물입니다. 이유는 $content 입니다.");
        }
      }
    }
    if (!found) {
      await _tts.speak("선택하신 약은 노인 금기 항목에 포함되지 않습니다.");
    }
  }
      ...
      body: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.elderly, color: Colors.orangeAccent, size: 64),
