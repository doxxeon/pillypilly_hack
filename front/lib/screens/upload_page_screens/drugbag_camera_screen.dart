...
/// 약봉투 촬영용 가로형 카메라 화면
class DrugbagCameraScreen extends StatefulWidget {
  const DrugbagCameraScreen({Key? key}) : super(key: key);

  @override
  State<DrugbagCameraScreen> createState() => _DrugbagCameraScreenState();
}

class _DrugbagCameraScreenState extends State<DrugbagCameraScreen> {
  ...
  void initState() {
    super.initState();
    // 가로 모드 강제 회전 제거 - 사용자가 자유롭게 회전 가능
    _initializeCamera();
  }
  ...
  Future<void> _initializeCamera() async {
    try {
      ...
      if (mounted) {
        ...
        await _announce("약봉투 촬영 화면입니다. 약봉투를 가로로 길게 화면에 맞춰주세요. 만약 결과가 나오지 않는다면 약 봉투를 180도로 돌려서 다시 한 번 촬영해주세요.");
      }
    } catch (e) {
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      child: Container(
                        ...
                        child: Semantics(
                          label: "약봉투를 가로로 길게 화면에 맞춰주세요. 만약 결과가 나오지 않는다면 약 봉투를 180도로 돌려서 다시 한 번 촬영해주세요.",
                          child: Column(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Text(
                                "약봉투를 가로로 길게 화면에 맞춰주세요",
                                style: TextStyle(
                                  color: Colors.white,
                              ...
                              Text(
                                "만약 결과가 나오지 않는다면 약 봉투를 180도로 돌려서 다시 한 번 촬영해주세요",
                                style: TextStyle(
                                  color: Colors.white,
