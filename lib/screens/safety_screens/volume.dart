...
class DoseContraScreen extends StatefulWidget {
  const DoseContraScreen({Key? key}) : super(key: key);

  @override
  State<DoseContraScreen> createState() => _DoseContraScreenState();
}

class _DoseContraScreenState extends State<DoseContraScreen> {
  ...
  Future<void> _startIntro() async {
    ...
    await _tts.speak("용량 금기 약물 확인 화면입니다. 약 이름을 말씀해주세요.");
  ...
  /// ✅ 용량금기 확인
  Future<void> _checkDoseContra(String input) async {
    ...
    await Vibration.vibrate(duration: 120);
    await _tts.speak("${matched['itemName']}을 선택하셨습니다. 용량 금기 정보를 불러옵니다.");
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _analyzeDoseData(results);
      } else {
  ...
  /// 📊 용량금기 분석
  Future<void> _analyzeDoseData(Map<String, dynamic> results) async {
    bool found = false;
    for (var entry in results.values) {
      final detail = entry['dur']?['doseTaboo'] ?? [];
      if (detail.isNotEmpty) {
        found = true;
        for (var d in detail) {
          final name = d['ITEM_NAME'] ?? '약물명 미상';
          final content = d['DOSE_CONTENT'] ?? '금기 용량 정보 없음';
          await _tts.speak("$name은 용량 금기 약물입니다. $content");
        }
      }
    }
    if (!found) {
      await _tts.speak("선택하신 약은 용량 금기 항목에 포함되지 않습니다.");
    }
  }
      ...
      body: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.scale, color: Colors.deepPurpleAccent, size: 64),
