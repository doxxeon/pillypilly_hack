...
class PeriodContraScreen extends StatefulWidget {
  const PeriodContraScreen({Key? key}) : super(key: key);

  @override
  State<PeriodContraScreen> createState() => _PeriodContraScreenState();
}

class _PeriodContraScreenState extends State<PeriodContraScreen> {
  ...
  Future<void> _startIntro() async {
    ...
    await _tts.speak("투여 기간 금기 약물 확인 화면입니다. 약 이름을 말씀해주세요.");
  ...
  /// ✅ 투여기간금기 확인
  Future<void> _checkPeriodContra(String input) async {
    ...
    await Vibration.vibrate(duration: 120);
    await _tts.speak("${matched['itemName']}을 선택하셨습니다. 투여 기간 금기 정보를 불러옵니다.");
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await _analyzePeriodData(results);
      } else {
  ...
  /// 📊 투여기간금기 분석
  Future<void> _analyzePeriodData(Map<String, dynamic> results) async {
    bool found = false;
    for (var entry in results.values) {
      final detail = entry['dur']?['periodTaboo'] ?? [];
      if (detail.isNotEmpty) {
        found = true;
        for (var d in detail) {
          final name = d['ITEM_NAME'] ?? '약물명 미상';
          final content = d['PERIOD_CONTENT'] ?? '금기 기간 정보 없음';
          await _tts.speak("$name은 투여 기간 금기 약물입니다. $content");
        }
      }
    }
    if (!found) {
      await _tts.speak("선택하신 약은 투여 기간 금기 항목에 포함되지 않습니다.");
    }
  }
      ...
      body: Center(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              const Icon(Icons.timer, color: Colors.tealAccent, size: 64),
              const SizedBox(height: 20),
              const Text(
                "투여 기간 금기 약물 검사 화면입니다.\n음성으로 약 이름을 말씀해주세요.",
